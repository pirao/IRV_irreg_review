...
import numpy as np
from utils.source import calculate_metrics
...
def obj_lasso(trial):
    h_alpha = trial.suggest_float("alpha", low=0, high=2,step=0.05)
...
def obj_elastic_net(trial):
    e_alpha = trial.suggest_float("n_alphas", low=0.0, high=2, step=0.05)
    e_l1_ratio = trial.suggest_float("l1_ratio", low=0.0, high=1,step=0.05)
...
def obj_random_forest(trial):
    f_n_estimators = trial.suggest_int("n_estimators", low=60,high=600,step=20)
    f_max_depth = trial.suggest_int("max_depth", low=1,high=6,step=1)
    f_min_samples_leaf = trial.suggest_int("min_samples_leaf", low=1,high=6, step=1)
    f_min_samples_split = trial.suggest_int("min_samples_split", low=2,high=6, step=1)
    
    f_oob_score = trial.suggest_categorical("bootstrap",[True,False])
    f_bootstrap = f_oob_score if False else True # necessary for the model to work
...
def obj_extra_trees(trial):
    e_n_estimators = trial.suggest_int("n_estimators", low=60, high=600, step=20)
    e_max_depth = trial.suggest_int("max_depth", low=1,high=6,step=1)
    e_min_samples_split = trial.suggest_int("min_samples_split", low=2,high=6, step=1)
    e_min_samples_leaf = trial.suggest_int("min_samples_leaf", low=1,high=6, step=1)
...
def obj_ada_boost(trial):
    h_n_estimators = trial.suggest_int("n_estimators", low=60, high=600, step=20)
    h_learning_rate = trial.suggest_float("learning_rate", low=0.001, high=0.1,step=0.005)
...
def obj_XGBRegressor(trial):
    x_max_depth = trial.suggest_int("max_depth",2,12)
    x_learning_rate = trial.suggest_float("learning_rate",0.001,0.1,step=0.005)
    x_n_estimators = trial.suggest_int("n_estimators", low=60, high=600, step=20)
    x_min_child_weight =  trial.suggest_int("min_child_weight",1,8)
    
    model = xgboost.XGBRegressor(n_estimators = x_n_estimators,
...
def obj_KNeighborsRegressor(trial):
    h_n_neighbors = trial.suggest_int("n_neighbors", 1, 10)
    h_weights = trial.suggest_categorical("weights", ['uniform','distance'])
    distance_metric =trial.suggest_categorical("distance_metric", ['euclidean','manhattan'])
    
    model = neighbors.KNeighborsRegressor(n_neighbors=h_n_neighbors,weights=h_weights,metric=distance_metric)
...
def obj_SGDRegressor(trial):
    sdg_loss = trial.suggest_categorical("sdg_loss", ['squared_loss', 'huber', 'epsilon_insensitive'])
    sdg_penalty = trial.suggest_categorical("penalty", ['l1', 'l2', 'elasticnet'])
    sdg_alpha = trial.suggest_float('alpha',low=0,high=2,step=0.05)
    
    model = linear_model.SGDRegressor(penalty=sdg_penalty, loss=sdg_loss,alpha=sdg_alpha,verbose=0)
...
def obj_catBoostRegressor(trial):
    cat_n_estimators = trial.suggest_int("n_estimators", low=60, high=600, step=20)
    cat_loss = trial.suggest_categorical("MAE", ['squared_loss', 'MAPE', 'Poisson'])
    #cat_depth = trial.suggest_int("depth",4,12)
...
def obj_LightBoost(trial):
    light_estimators = trial.suggest_int("num_estimators", low=60,high=600,step=20)
    #light_max_depth = trial.suggest_int("max_depth", low=4,high=12,step=1)
